...
| **Forge** (v2.17) | The timed idea-generation / goal-setting session for a goals day or a quarterly priority reset — **not** part of the weekly meeting. A **room** view (projector) and a phone **player** view (`?forge=FS-###`) share one server-timed state: opener → anonymous Diverge rounds → Relay (build on a dealt card) → Cluster → blind Vote → Investment committee → Claim → Handoff contracts → the goal form with a SMART meter → two Goal-doctor rounds → Commit → **Lock** (writes each person's `<Name> — FY27 Goals` tab, creates priorities from the Q1 milestones, parks leftovers as initiatives; never a to-do). Plus **Timed write** ("next N minutes, everyone write…"), the **wheel** (random person, with photos — also on the huddle timer bar) and a **passphrase gate** (`FORGE_PASSWORD`). Tabs: `L10_Forge_*`, `L10_Goals`. See [`IDEA-FORGE.md`](./IDEA-FORGE.md) |
