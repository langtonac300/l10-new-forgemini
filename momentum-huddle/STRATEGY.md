...
> The one page in Momentum Huddle that is **not** part of the weekly meeting. It
...
|---|---|---|
| Priorities (`L10_Rocks`) | one owner, one due date, Shift + Accounts tags | A quarterly commitment. An idea that plays out across five accounts over two quarters doesn't fit one row |
...
| Issues / Solve | this week's decisions | Transient |
...
individual tests; Priorities stay the quarterly commitments; the initiative links to both.
...
| `Rock ID` | The Priority created from this cell, if any |
...
and priority promotion, plus whatever a person types.
...
6. **Initiative drawer** (right-hand sheet): kicker + title, Edit / Close; chips
   ...
   Promote to priority, plus Add-account chips; N/A rows dimmed last) ·
...
| `l10_promoteInitiativeToRock(id, account, p)` | creates a Priority via `l10_addRock` with `Source = SI-###`, stores the id on the cell |
...
Same logic in `l10-huddle/`; only the words differ (Priorities ↔ Rocks). The `L10_*`
