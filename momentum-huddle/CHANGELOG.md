...
word wherever colour carries meaning. `rockLine()` is untouched — the huddle's Priority
...
[`design/ux-2026-09-25/`](./design/ux-2026-09-25/README.md).
...
- **Quarter card.** Day *n* of *N* · days left in the quarter, a bar, and "*x* of *y*
  ...
  today, on any priority that wasn't dropped). **Status** chips — ▲ off track · ◆ need a
  check · ○ no plan yet · ● on track, each a filter with its count — and **Owner** chips with
  each person's open-priority load. **Group** by Status (default) or Owner, remembered per
...
- **One table, one quarter axis.** Month ticks and a Today marker in a sticky header; every
  ...
  the same `l10_setMilestoneStatus` path (the last one still auto-marks the priority DONE).
...
- **Milestone panel under the open row:** tick, **+7d**, click the date to re-date, ✎ rename /
  ...
  definition of done; caveat; the linked metric as **13 weekly bars** (dark = on goal, light =
...
- **Done this quarter** folds under the table (status set to DONE on or after the quarter
  start), ↩ Reopen. Priorities finished in earlier quarters no longer list on this page —
  they stay in the tab.
- **New priority sheet** (+ New priority): the outcome + how you'll know it's done, owner
...
- **Harness:** fixtures cover every state (8 active priorities plus done, dropped and
  ...
  done this quarter, split → sheet, the New priority and edit payloads, Send to Issues, the
...
- **Re-paste:** `L10Setup.gs`, `L10Code.gs`, `L10Css`, `L10Js`. Then run **Momentum Huddle →
...
- Out of scope, parked: the huddle's Priority review segment keeps the old line (its quick
...
- **Run sheet:** [`FORGE-RUN-SHEET.md`](./FORGE-RUN-SHEET.md) — the facilitator's one page
  for Thursday 2026-09-24.
- **Re-paste:** `L10Setup.gs`, `L10Forge.gs`, `L10ForgeJs.html`, `L10ForgeCss.html`. Then
  run **Momentum Huddle → Setup / repair tabs** once (this is what upgrades the `FORGE_*` values
...
- **Metrics capture: the "N value(s) could not be read — details below the capture
  button" toast pointed at nothing.** `doCapture` wrote the per-metric reasons into the
  capture card, then re-rendered the Metrics page and the segment — which rebuilt the card
...
- **One compact row** per to-do: state glyph (○ ◐ ⏸, click to change), text +
  ...
  priority, take it to Solve, drop) and ✓ Done.
...
- **Drawer** (right-hand sheet) per to-do: owner, state pill, due in words, the
  ...
  it a priority · Take it to Solve · Drop. Done / drop closes it.
...
X", "this worked on Brady US, port it to Seton"). Priorities are one owner + one
...
- **Drawer** per initiative: editable thesis / origin / quarter / notes, the
  ...
  test** (the hub's Ideas tab, via the same writer Solve uses — now
  `l10HubIdeaAppend_`) and **↑ priority** (`l10_addRock` with `Source = SI-###`),
...
`L10Js.html`; run **Momentum Huddle → Setup / repair tabs** once (adds `L10_Team`);
redeploy.
...
- **As scored in the huddle:** the `Todo Done %` the room saw at Wrap-up, last /
...
- **Where to-dos come from** (issue / priority / email / direct / ↻ weekly — from the
...
`L10Css.html`, `L10Js.html`, `L10Setup.gs`; run **Momentum Huddle → Setup / repair
