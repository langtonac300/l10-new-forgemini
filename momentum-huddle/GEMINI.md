...
2. **Momentum Huddle → Gemini → Set API key…** — stored in the `L10_GEMINI_API_KEY`
   script property, **never in the sheet**.
3. **Momentum Huddle → Gemini → Test connection** — verifies the key reaches the
   configured model and reports today's call count.
4. Optional: `L10_Config` → `GEMINI_MODEL`, `GEMINI_DAILY_CAP`.

**Momentum Huddle → Gemini → Status / usage** reports on/off, model, and calls used
...
first. Candidates considered, none selected yet: Solve root-cause and playbook
suggestions · to-do polish and meaning-based dedupe · recap/summary drafting
(the guard above is what would make this safe) · metrics movement narration ·
...
this one and `l10-huddle/` — since it is logic, not wording.
