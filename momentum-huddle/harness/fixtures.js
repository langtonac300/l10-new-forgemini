...
(function () {
  ...
  // Priorities fixtures (v2.19 page) are relative to today, plus the start of
  ...
  const PLAN = {
    ...
    // Every state the Priorities page draws, counted so the smoke can assert it
  ...
  window.__FIXTURES = {
    ...
    l10_addRock: function (p) {
      idSeq++;
      p = p || {};
      return { ok: true, id: 'RK-' + idSeq, row: rk('RK-' + idSeq, p.title || 'New priority', p.owner || 'Alex', TODAY, p.due || '', 'ON TRACK',
