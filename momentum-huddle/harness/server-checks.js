...
// --- Priorities page v2.19: status why / was / confirmed / caveat --------------------
...
function rockEnv(headers) {
  ...
  const row = (id, status, extra) => H.map((h) => (extra && h in extra) ? extra[h] : ({ 'ID': id, 'Rock': 'Priority ' + id, 'Owner': 'Alex', 'Due': '2026-10-31',
...
function scenarioRockConfirm() {
  ...
  check(res.ok === false && /on-track/.test(res.error || ''), 'rock confirm: an off-track priority cannot be confirmed on track: ' + JSON.stringify(res));
  freshExecution(env);
...
function scenarioRockPreRepair() {
  ...
  freshExecution(env);
  res = env.ctx.l10_editRock('RK-001', { text: 'Priority RK-001', owner: 'Alex', due: '2026-10-31', caveat: 'context' });
...
function scenarioRockEditAndCascade() {
  ...
  // Finishing the last milestone marks the priority done through the same status writer.
  freshExecution(env);
  ...
  check(res.ok && res.rockDone === 'RK-003' && r['Status'] === 'DONE' && r['Previous Status'] === 'ON TRACK' && r['Status Updated'] === TODAY_S,
    'milestone cascade: the priority should go DONE with was=ON TRACK, got ' + JSON.stringify(r));
  freshExecution(env);
