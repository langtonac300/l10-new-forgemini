...
// Plan: priorities + their milestones + the playbook + the strategy tabs. Six
// tab reads (the three strategy tabs are small); missing tabs (pre-setup) read
// as zero rows.
function l10BootPlan_() {
  return {
    ...
    // The Priorities page's why / was / confirmed / caveat columns (v2.19): false
...
// quick flip doesn't ask; the Priorities page does).
function l10_setRockStatus(id, status, opts) {
  ...
  if (!row) return { ok: false, error: 'Priority ' + id + ' not found.' };
  ...
  if (reason && l10ReadTab_(L10.TABS.ROCKS).headers.indexOf('Off Track Reason') === -1) {
    return { ok: false, error: 'L10_Rocks has no Off Track Reason column yet — run Momentum Huddle → Setup / repair tabs once, then retry.' };
  }
...
function l10_restoreRockStatus(id, snap) {
  ...
  return wrote ? { ok: true } : { ok: false, error: 'Priority ' + id + ' not found.' };
}
...
function l10_confirmRock(id, value) {
  if (l10ReadTab_(L10.TABS.ROCKS).headers.indexOf('Confirmed On') === -1) {
    return { ok: false, error: 'L10_Rocks has no Confirmed On column yet — run Momentum Huddle → Setup / repair tabs once, then retry.' };
  }
  var row = l10RockRow_(id);
  if (!row) return { ok: false, error: 'Priority ' + id + ' not found.' };
  var v = value === undefined ? l10Today_() : String(value || '').slice(0, 10);
  if (v && !/^\d{4}-\d{2}-\d{2}$/.test(v)) return { ok: false, error: 'Bad date.' };
  if (v && String(row['Status']) !== 'ON TRACK') return { ok: false, error: 'Only an on-track priority can be confirmed on track.' };
...
function l10_editRock(id, p) {
  ...
  // Same only-when-sent rule for the Priorities page's fuller edit form.
  if (p.caveat !== undefined) {
    ...
    if (caveat && l10ReadTab_(L10.TABS.ROCKS).headers.indexOf('Caveat') === -1) {
      return { ok: false, error: 'L10_Rocks has no Caveat column yet — run Momentum Huddle → Setup / repair tabs once, then retry.' };
    }
...
// The one writer for the hub's Ideas tab (Solve's "Make it a test" and the
...
function l10InitNotReady_() {
  return { ok: false, error: 'Strategy tabs are missing — run Momentum Huddle → Setup / repair tabs once, then retry.' };
}
...
function l10_setInitiativeAccount(p) {
  ...
  if (p.rockId !== undefined && u['Rock ID']) what.push('priority ' + u['Rock ID']);
...
// "Promote to priority": a quarterly commitment for one account, born from the
// initiative. The priority's Source column carries the SI id (tappable in the
...
function l10_promoteInitiativeToRock(id, account, p) {
  ...
  if (!rk || rk.ok === false) return rk || { ok: false, error: 'Could not create the priority.' };
  var cell = acct ? l10_setInitiativeAccount({ initiativeId: id, account: acct, rockId: rk.id }) : null;
  if (!cell) l10InitiativeTouch_(id, 'Promoted to priority ' + rk.id);
