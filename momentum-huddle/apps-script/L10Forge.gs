// Momentum Huddle — Forge: the timed, multi-device idea-generation and
...
// "<Name> — FY27 Goals" tab, creates priorities from the Q1 milestones, and
...
function l10ForgeNotReady_() {
  return { ok: false, error: 'The Forge tabs are not in this workbook yet — run Momentum Huddle → Setup / repair tabs once, then reload.' };
}
...
function l10ForgeLockPlan_(sessionId) {
  ...
  Object.keys(people).forEach(function (person) {
    ...
    biz.forEach(function (g) {
      if (!g.ms1 || !g.ms1Due) plan.warnings.push(person + ' · "' + g.title + '": no dated Q1 milestone, so no priority will be created.');
...
function l10_forgeLock(sessionId, who) {
  ...
  plan.people.forEach(function (entry) {
    ...
    entry.items.forEach(function (item) {
      ...
      if (item.rock && !g.rockId) {
        ...
        } else errors.push(entry.person + ' · ' + g.title + ': priority not created — ' + ((res && res.error) || 'unknown'));
      }
  ...
  try {
    l10NotifyChat_('Forge — ' + String(s['Title'] || sessionId) + ': ' + ideasN + ' ideas · ' + plan.goals + ' goals locked · ' +
      rocks + ' priorities created · ' + parked + ' parked' + (avg !== null ? ' · rated ' + avg + '/10' : ''));
  } catch (e2) {}
