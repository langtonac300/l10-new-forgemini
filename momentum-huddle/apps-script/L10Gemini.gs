// Momentum Huddle — Gemini service layer.
...
//   2. Momentum Huddle -> Gemini -> Set API key...     (stored in script properties)
//   3. Momentum Huddle -> Gemini -> Test connection    (verifies the key + the model)
...
function l10GeminiGenerate_(prompt, opts) {
  opts = opts || {};
  var s = l10GeminiSettings_();
  if (!s.key) return { ok: false, off: true, error: 'No Gemini API key set (Momentum Huddle > Gemini > Set API key).' };
