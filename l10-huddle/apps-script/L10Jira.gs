...
//   2. L10 Huddle -> Jira -> Set API token…    (stored in script properties)
//   3. L10 Huddle -> Jira -> Test connection     (verifies + lists project keys)
//   4. L10 Huddle -> Jira -> Sync now, then Turn on auto-sync (10-min trigger).
//
...
function l10JiraTodoFields_(s, todo) {
  ...
  var parts = ['Created from the Paid Media L10 huddle.'];
...
function l10JiraSyncTodos() {
  ...
  if (!l10JiraEnabled_(s)) {
    l10JiraToast_('Jira sync is off — set JIRA_DOMAIN, JIRA_PROJECT_KEY, JIRA_EMAIL in L10_Config and the API token (L10 Huddle ▸ Jira ▸ Set API token).');
