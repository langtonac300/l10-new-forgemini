...
[`IDEA-FORGE.md`](../momentum-huddle/IDEA-FORGE.md) v2. Both copies; harness green in both (the suite
...
- **Lock** previews everything first, then: writes each person's goals into their own
  ...
  scanning column B, never assumed), creates a rock from every dated Q1 milestone
...
- **Re-paste:** create three new files in the Apps Script editor — `L10Forge.gs`,
  ...
  `L10Js.html`. Run **L10 Huddle → Setup / repair tabs** once (adds the five tabs and
...
configured, **L10 Huddle → Gemini →** submenu with Set API key / Test connection /
Status.
