# L10 Huddle — EOS Level 10 weekly meeting app for the paid media team
...
huddle EOS-Level-10 style: Segue → Scorecard → Rock review → Headlines → To-do
list → IDS → Conclude, with hard timeboxes, a 13-week scorecard, and everything
written back to `L10_*` tabs as the audit trail.

**Built 2026-06-12** (requested by Alex: "take the weekly huddles to the next
level — very very similar to EOS Level 10"). Natural home: the **Tuesday 10:00
Weekly Task Review** (moved from Monday as of the week of Jun 15 2026 — see current
state below). Stuart already likes the EOS framework (his "GSD" format),
...
> GA4 scorecard connector (`L10Ga4.gs`), an in-app metric builder, the calm Issue-Solving
...
> [`apps-script/L10Guide.html`](./apps-script/L10Guide.html), menu **L10 Huddle →
> New member guide**.**
...
resurface ad hoc. EOS L10 supplies the missing structure, and the app removes
the discipline cost of running it: timers keep 5-minute segments at 5 minutes,
the scorecard auto-pulls what can be pulled, and the Conclude segment generates
...
| **Data-source health gate** (v2.9) | The warehouse scores every upstream the scorecard depends on (`v_l10_data_health` in Alex's dataset + a live SP-vs-SB mart check) and the app warns the room BEFORE a number is read: red strip naming sick sources, ⛔ "source stale — do not read as current" on mapped metrics in grid and capture. Leads gated ≤8 days behind the spend clock — the false −58% scare can't recur silently. See [`sql/l10-data-health.sql`](./sql/l10-data-health.sql) + `L10Health.gs`. |
| **Meta Monday cascade generator** | Conclude builds copy-paste talking points in Stuart's lens order, with `___` blanks where a live number must come from the source tool (never invents numbers) |
...
| **Four-shifts rock tags** | Rocks tag to Alex's FY27 strategic framing (Shift 1–4) + account tags in the team's vocabulary |
| **Rocks page on one quarter axis** (v2.19) | The page leads with what needs attention: a quarter card (day *n* of *N*, milestones due so far vs done, status + owner filters), then one table where every rock's milestones sit on the **same quarter axis** — off track (with the owner's one-line **why**), needs a check (the milestones disagree with "on track"; the owner answers **Yes, still on track** or flips it), no plan yet, on track. The open row shows its milestones, definition of done, caveat and the linked metric's 13 weekly bars; the composer is a side sheet. Four columns appended to `L10_Rocks` (run Setup / repair tabs once). See [`CHANGELOG.md`](./CHANGELOG.md) v2.19 |
| **Rock milestones on a to-scale timeline** (v1.4) | Each rock carries dated mini-milestones plotted proportionally across its life (Created → Due; 90-day EOS default when no due date): tick them off in the huddle, the last one auto-marks the rock DONE; labels lane-pack so close dates never overlap |
...
| **Forge** (v2.17, cut to the goals day in v2.18) | The timed idea-generation / goal-setting session for a goals day or a quarterly rock reset — **not** part of the weekly meeting. A **room** view (projector) and a phone **player** view (`?forge=FS-###`) share one server-timed state: the wall starts seeded with the FY27 goals and the team backlog (`FORGE_SEED_CARDS`) → one round of new ideas → Claim (grouped by the FY27 goal each card serves) → the goal form with a SMART meter → one peer review → Commit → **Lock** (writes each person's `<Name> — FY27 Goals` tab, creates rocks from the Q1 milestones, parks leftovers as initiatives; never a to-do). Plus **Timed write** ("next N minutes, everyone write…"), the **wheel** (random person, with photos — also on the huddle timer bar) and an optional **passphrase gate** (`FORGE_PASSWORD`, off by default; v2.18 cut the day to these steps — the opener, relay, cluster, vote, committee and handoff phases still run when `FORGE_PHASES` lists them). Tabs: `L10_Forge_*`, `L10_Goals`. See [`IDEA-FORGE.md`](../momentum-huddle/IDEA-FORGE.md) |
...
| **Stale to-dos get re-filed, not nagged** (v2.7) | At 3 carry-overs (`TODO_STALE_CARRIES`) a row is flagged once with two doors — **make it a rock** or **take it to IDS**. A 7-day commitment that survives three weeks is misclassified work, not a lazy owner. The carry counter itself moved out of Conclude into an idempotent weekly sweep, so skipping Conclude no longer freezes the staleness signal |
| **Kudos capture in Segue** | Matches the room's hype-man culture; kudos log to `L10_Headlines` |
| **To-do → team-chat ping** (v1.11) | Adding or completing a to-do posts `L10 To-Do - Added/Complete - {Owner} - {Task}` to the *Paid Team & Stuart* space via an incoming webhook — async visibility (esp. for Stuart, who's never in the room). Opt-in via `CHAT_WEBHOOK_URL`; **live since 2026-06-24** |
...
| **Custom digests** (v2.2) | On top of that, anyone can add **their own scheduled emails** in **Settings → Custom digests** — fully customizable per analyst on **content** (to-dos / rocks / scorecard / headlines) **× frequency** (daily / weekdays / weekly) **× time of day**, mix-and-match. Opt-in `L10_Digests` tab; hourly `l10RunDigests` sends on a sheet-timezone clock-hour match, deduped, empty-skipped. See [`EMAIL-AUTOMATION.md`](./EMAIL-AUTOMATION.md) |
| **Seeded with real state** | Rocks from the FY27 goal drafts (flagged "confirm with analyst"), issues from the live problem list (PDC feed, LinkedIn feed, Amazon mapping, NB lead decline, unattached neg lists, Seton stand-up) |
| **Schedule to Google Calendar** (v1.22) | **📅 Schedule** on every issue, headline, and to-do (and the New-to-do form), in the header, and inside IDS opens a calendar.google.com-style day view: the item's text prefills the event title, busy blocks grey out (own events titled, invitees anonymous), click/drag to pick an open slot, invite the room, and it writes the event via the Advanced Calendar service. See [`CALENDAR.md`](./CALENDAR.md) |
| **Quick add from the sheet menu** (v1.21) | **L10 Huddle → Add headlines… / Add issues… / Add to-dos… / Add rocks… / Update scorecard…** — batch-entry dialogs with the app's full form features (multi-owner to-do chips, cascade flag, capture grid) for between-huddle upkeep without opening the app |
| **To-do → source-IDS context** (v1.21) | A to-do born in IDS carries a tappable **`from IS-### ↗`** chip → read-only modal with the issue + its Identify/Discuss/Solve notes and sibling to-dos — a week-old assignment explains itself |
...
| **Turn-order IDS voting** (v1.21) | "Start voting round": random first voter, around the room in turn, 3 votes each; the list freezes during the round and auto-sorts by vote total when the room's done |
| **Pre-huddle brief docket** (v1.20) | A ranked "what deserves IDS time" list ($-at-stake, evidence, caveats attached) appears on the start screen when something POSTs it in via the [`doPost` contract](./BRIEF-INTAKE.md); one tap promotes an item to a real issue with the evidence in its Identify notes. (The scheduled `tuesday-pre-brief` poster was retired 2026-07-09 — the docket arrives only if a session or future automation POSTs the JSON) |
| **Analysis playbook + "seen before"** (v1.20) | Typing an issue live-matches the team's standing reports (NB pullback, S-curve read, serving-URL audit, Keep/Kill, hub test — `L10_Playbook`) and past decisions ("IS-014, March — solved: …"), so IDS starts from the record instead of memory |
| **Decision ledger + outcome loop** (v1.20) | Solved issues come due for a "did the fix hold?" verdict in Conclude (`Review On`, default +4 weeks); verdicts build the searchable ledger on History — the anti-re-litigation memory |
| **1:1 pages + "My lens"** (v1.20) | A per-person 1:1 page (parked issues, their to-dos/rocks, issues they raised — matches the analyst-led 1:1 format; deliberately no per-person completion %) and a per-browser lens chip that leads lists with your own items |
...
│ • 13-wk scorecard + sparklines  │   └───────────────────────────────┘
│ • IDS voting / solve / park     │
...
1. Open the workbook → **Extensions → Apps Script**. Add files and paste:
   ...
   item; `L10QuickAdd` powers the menu's Add headlines/issues/to-dos/rocks + Update
   scorecard dialogs; `L10Calendar.gs` is optional — only for **📅 Schedule** meeting
   ...
   only for GA4 scorecard metrics; it needs the **Analytics Data API** enabled in the
...
3. Run **`l10Setup`** once from the editor (or reload → **L10 Huddle → Setup /
...
seven segments with countdown timers. Scorecard and Rocks are read-only
ON/OFF-track calls — anything off track gets "dropped down" to Issues with one
click. IDS: vote, expand the top issue, then ① Identify (owner names the root
cause) → ② Discuss (notes) → ③ Solve (one-line decision + to-dos; "＋ Add now"
saves a to-do without waiting for a solve). Conclude: copy the recap to the
...
- **Seeded rocks are flagged as drafts** — they mirror
...
| [`apps-script/L10Ga4.gs`](./apps-script/L10Ga4.gs) | **GA4 scorecard connector** (new, v2.0): a metric with the `GA4` source pulls sessions/users/revenue/… from the Google Analytics Data API using each viewer's own sign-in — no tokens stored. Needs the Analytics Data API + `analytics.readonly`; property ID in `GA4_PROPERTY_ID`. |
| [`apps-script/L10QuickAdd.html`](./apps-script/L10QuickAdd.html) | The quick-add dialog (v1.21): one templated modal serving the menu's **Add headlines/issues/to-dos/rocks** batch forms + **Update scorecard** capture grid — rows funnel through the app's own add functions via `l10_quickAdd` |
| [`apps-script/L10Mail.gs`](./apps-script/L10Mail.gs) | Email automation: personalized Monday heads-up, Gmail reply-ingest of issues/headlines/rocks, Tuesday recap, per-analyst **custom digests** (hourly `l10RunDigests`, sheet-timezone match), triggers + menu items |
...
| [`apps-script/L10Stats.gs`](./apps-script/L10Stats.gs) | **Team stats feed** (v2.12): `l10_teamStats` reads the whole `L10_Todos` tab on demand (not the 60-day boot window) and ships one compact row per to-do plus every concluded huddle's `Todo Done %`; all arithmetic is client-side (`teamStatsCompute_` in `L10Js.html`). Read-only. |
| [`apps-script/L10Jira.gs`](./apps-script/L10Jira.gs) | One-way **L10 → Jira** to-do sync: OPEN to-do → Jira issue, DONE to-do → issue transitioned to Done; idempotent via the `Jira Key` column; 10-min trigger + **Jira** submenu. Token in script property, settings in `L10_Config`. See the *Jira sync* section above. |
...
| [`apps-script/L10Guide.html`](./apps-script/L10Guide.html) | The visual new-member onboarding one-pager, opened in a modal from **L10 Huddle → New member guide** (`l10OpenGuide`) — and, since v2.8, from the **? button in the app's nav** (served by `l10_getGuideHtml` into an iframe). Byte-identical to [`../../processes/l10-new-member-guide.html`](../../processes/l10-new-member-guide.html) (the standalone shareable copy) — **keep the two in sync**. |
...
| [`apps-script/L10Health.gs`](./apps-script/L10Health.gs) | **Data-source health** (v2.9, optional): reads `v_l10_data_health` via the BigQuery advanced service + the SP-vs-SB Amazon mart-block check (INFORMATION_SCHEMA-discovered, drift → UNKNOWN). Cached ~30 min; feeds the scorecard strip + per-metric ⛔ flags; **L10 Huddle → Data health check** menu. Off silently when the service or config is absent. |
...
> sparklines) · #8 v1.16 (corporation-grade emails) · #9 v1.17 (scorecard health roll-up + a11y) ·
> **#5's render-helper/pill refactor + #7 guide/first-run + #10 quiet-feedback/timer/surgical-revert
> v2.8**, plus the honorable-mention IDS dialog semantics, the `.ms-tl` ResizeObserver fix,
...
keyboard-operable controls, (3) undo-on-toast + a Conclude confirm. Everything traces back to
...
~~a 1:1 view for the parked-issues lists~~ (built, v1.20) · per-quarter rock
rollover ceremony · meeting-notes export to Drive · evidence-suggested rock
