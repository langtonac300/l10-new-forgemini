...
(async () => {
  ...
  await clickNav(page, 'todos');
  // Scoped to the To-dos page: rocks carry SI references too (RK-007), on a hidden page.
  ...
  // --- Rocks page (v2.19 layout): quarter card, shared axis, buckets, panel, status, sheet ---
  {
    ...
    if ((await page.$$('.rkr')).length !== 8) errors.push('rocks page should list the 8 active fixture rocks, got ' + (await page.$$('.rkr')).length);
    ...
    if (!(await page.$('.qturn [data-qsplit="RK-011"]'))) errors.push('the past-quarter rock should keep its quarter-turn card');
    ...
    if (!/2 rocks of 8|1 rock of 8/.test(await page.$eval('.rkt-count', (el) => el.textContent))) errors.push('count label should say "of 8" while filtered');
    ...
    await page.waitForTimeout(120);
    if ((await gsSince(nC)).some((c) => c.fn === 'l10_setRockStatus')) errors.push('an empty why must not mark the rock off track');
    ...
    // Split on the quarter-turn card prefills the New rock sheet.
    await page.click('[data-qsplit="RK-011"]');
    await page.waitForTimeout(150);
    if (!(await page.$eval('#rk-new', (el) => el.style.display === 'block'))) errors.push('split did not open the New rock sheet');
    else {
      ...
      await page.waitForTimeout(120);
      if (await page.$eval('#rk-new', (el) => el.style.display !== 'none')) errors.push('Escape did not close the New rock sheet');
    }
    // New rock sheet: title, owner, account; due defaults to the end of the quarter; the new row opens.
    await page.click('[data-rknew]');
    await page.waitForTimeout(150);
    await page.fill('#rk-new .js-rkn-title', 'Harness rock');
    ...
    if (!addRk || addRk.args[0].title !== 'Harness rock' || addRk.args[0].owner !== 'Courtney' || addRk.args[0].accounts !== 'Amazon' || addRk.args[0].due !== qEnd) {
      errors.push('New rock sent the wrong payload: ' + JSON.stringify(addRk && addRk.args[0]));
    }
    if (await page.$eval('#rk-new', (el) => el.style.display !== 'none')) errors.push('New rock sheet stayed open after adding');
    const openRow = await page.$eval('.rkr--open', (el) => el.dataset.rkid).catch(() => '');
    if (!/^RK-\d+$/.test(openRow) || openRow === 'RK-006') errors.push('the new rock should open with its milestone line ready, open row: ' + openRow);
    ...
    // Send to Issues carries the rock id in the notes.
    nC = await gsN();
    ...
    await page.waitForTimeout(100);
    if (await page.evaluate(() => !!state.rkFilter.status)) errors.push('the room view should clear the rocks filter');
